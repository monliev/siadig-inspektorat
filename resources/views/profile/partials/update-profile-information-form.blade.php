<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div class="mt-4">
            ...
            <x-text-input id="jabatan" type="text" class="mt-1 block w-full bg-gray-100" :value="$user->jabatan ?? '-'" disabled />
        </div>
        
        {{-- Role --}}
        <div class="mt-4">
            <x-input-label for="role" :value="__('Role / Peran')" />
            <x-text-input id="role" type="text" class="mt-1 block w-full bg-gray-100" :value="$user->role->name ?? 'Belum Diatur'" disabled />
        </div>
        
        {{-- Entitas / OPD --}}
        <div class="mt-4">
            <x-input-label for="entity" :value="__('Unit Kerja')" />
            <x-text-input id="entity" type="text" class="mt-1 block w-full bg-gray-100" :value="$user->entity->name ?? 'Inspektorat Kabupaten Trenggalek'" disabled />
        </div>
