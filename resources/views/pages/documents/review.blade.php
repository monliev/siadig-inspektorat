                                ...
                                @forelse ($reviewDocuments as $document)
                                    <tr class="border-b">
                                        ...
                                        <td class="text-center py-3 px-4 flex items-center justify-center space-x-2">
                                            {{-- Tombol Approve --}}
                                            <form action="{{ route('documents.approve', $document->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menyetujui dokumen ini?');">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" class="text-sm bg-green-500 hover:bg-green-700 text-white py-1 px-2 rounded">Approve</button>
                                            </form>
                                            {{-- Tombol Reject --}}
                                            <form action="{{ route('documents.reject', $document->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menolak dokumen ini?');">
                                                @csrf
                                                @method('PATCH')
