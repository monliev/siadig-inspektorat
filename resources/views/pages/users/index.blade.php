                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('users.index') }}" method="GET" class="mb-4">
                        <div class="flex space-x-2">
                            <input type="text" name="search" placeholder="Cari nama, email, NIP, atau jabatan..." value="{{ $search ?? '' }}"
                            ...
                            <a href="{{ route('users.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-300">Reset</a>
                        </div>
                    </form>
                    ...
                    @endif
                    @if ($search)
                    <div class="mb-4"><p class="text-sm text-gray-700">Menampilkan hasil untuk: <span class="font-semibold">{{ $search }}</span></p></div>
                    @endif
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="text-center py-10 text-gray-500">
                                            @if ($search)
                                                Tidak ada pengguna yang cocok dengan kata kunci <span class="font-semibold">"{{ $search }}"</span>.
                                            @else
