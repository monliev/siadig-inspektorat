        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mt-4">
                        <p class="text-sm text-gray-600 mb-2">Permohonan Anda telah disetujui. Silakan unduh surat balasan Anda di bawah ini:</p>
                        <a href="{{ Storage::url($serviceRequest->final_document_path) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-500">
                ...
                <div class="p-6 text-gray-900">
                    ...
                    @foreach($serviceRequest->revisions as $revision)
                        <div class="border-t pt-3 mt-3">
                            ...
                            <blockquote class="mt-1 p-3 bg-gray-50 rounded-md border-l-4 border-gray-300">{{ $revision->notes }}</blockquote>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <p class="text-sm text-gray-600 mb-4">Silakan unggah ulang dokumen yang perlu diperbaiki sesuai catatan dari auditor.</p>
                    
                    <form action="{{ route('service-requests.submitRevision', $serviceRequest->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="space-y-6">
                            {{-- Loop semua dokumen yang pernah diunggah untuk permohonan ini --}}
                            @foreach($serviceRequest->uploadedDocuments as $document)
                            <div class="border-t pt-4">
                                <label for="revisi_doc_{{ $document->required_document_id }}" class="block font-medium text-sm text-gray-700">
                                    Perbarui: <strong>{{ $document->requirement->name }}</strong>
                                </label>
                                <input type="file" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none mt-2" 
                                    id="revisi_doc_{{ $document->required_document_id }}" 
                                    name="revisi_docs[{{ $document->required_document_id }}]">
                                <p class="text-xs text-gray-500 mt-1">Unggah file baru untuk menggantikan file sebelumnya.</p>
                            </div>
                            @endforeach
                        </div>
