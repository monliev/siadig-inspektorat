            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <table class="min-w-full bg-white">
                                ...
                                <td class="text-center py-3 px-4">
                                    ...
                                    @if ($disposition->status !== 'Selesai')
                                        <div x-data="{ open: false }">
                                            ...
                                            <div x-show="open" x-transition.opacity style="display: none;" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 p-4 text-left">
                                                <div @click.away="open = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
                                                    ...
                                                    <p class="text-sm text-gray-600 mb-4">Anda yakin ingin menutup disposisi untuk dokumen "{{ $disposition->document->title }}"?</p>
                                                    <form action="{{ route('dispositions.complete', $disposition->id) }}" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                                        <div>
                                                            <label for="closing_note_{{ $disposition->id }}" class="block text-sm font-medium text-gray-700">Catatan Penutupan (Opsional)</label>
                                                            <textarea name="closing_note" id="closing_note_{{ $disposition->id }}" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"></textarea>
                                                        </div>
                        ...
                        @empty
                            <tr><td colspan="6" class="text-center py-10">Tidak ada disposisi masuk.</td></tr>
                        @endforelse
                    </tbody>
                        </table>
