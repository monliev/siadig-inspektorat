        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                ...
                <div class="flex justify-between items-start flex-wrap gap-4">
                    ...
                    <div>
                        @if (($disposition->from_user_id == Illuminate\Support\Facades\Auth::id() || $disposition->to_user_id == Illuminate\Support\Facades\Auth::id()) && $disposition->status !== 'Selesai')
                        <div x-data="{ open: false }">
                            <button @click="open = true" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                            ...
                            <div x-show="open" x-transition.opacity style="display: none;" class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 p-4">
                                <div @click.away="open = false" class="bg-white rounded-lg shadow-xl p-6 w-full max-w-lg mx-4">
                                    ...
                                    <p class="text-sm text-gray-600 mb-4">Anda yakin ingin menutup disposisi ini? Aksi ini tidak dapat dibatalkan.</p>
                                    <form action="{{ route('dispositions.complete', $disposition->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <div>
                                            <label for="closing_note" class="block text-sm font-medium text-gray-700">Catatan Penutupan (Opsional)</label>
                                            <textarea name="closing_note" id="closing_note" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"></textarea>
                                        </div>
                                        <div class="flex justify-end space-x-2 mt-6">
                                            ...
                                            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Ya, Selesaikan</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    @forelse ($disposition->responses as $response)
                        <div class="border-t border-gray-200 pt-4">
                            <div class="flex justify-between items-center">
                                ...
                                <p class="text-xs text-gray-500">{{ $response->created_at->translatedFormat('d M Y, H:i') }}</p>
                            </div>
                                ...
                                <div class="mt-3">
                                    ...
                                    <ul class="mt-1 list-disc list-inside space-y-1">
                                        @foreach ($response->attachments as $attachment)
                                            <li>
                                                <a href="{{ asset('storage/' . $attachment->file_path) }}" target="_blank" class="text-sm text-blue-600 hover:underline">
