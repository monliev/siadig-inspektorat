            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <a href="{{ route('document-requests.show', $documentRequest->id) }}"
                        ...
                        Progres Permintaan</a>

                    <p class="text-sm text-gray-600">Untuk permintaan: <span
                            class="font-semibold">{{ $documentRequest->title }}</span></p>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white">
                            ...
                            <tbody class="text-gray-700">
                                    ...
                                    <td class="text-center py-3 px-4">
                                        ...
                                        <div class="flex items-center justify-center space-x-2">
                                            ...
                                            <a href="{{ route('client-submissions.show', $document->id) }}"
                                                class="text-sm bg-blue-500 hover:bg-blue-700 text-white py-1 px-2 rounded">Lihat</a>
                                            ...
                                            <form action="{{ route('documents.reject', $document->id) }}" method="POST"
                                                onsubmit="return confirm('Tolak dokumen ini?');">
                                                @csrf
                                                @method('PATCH')
                                                <input type="hidden" name="redirect_to" value="{{ url()->current() }}">
                                                <button type="submit"
                                            ...
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $document->status == 'Diarsip' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                                {{ $document->status }}
                                            </span>
                                            @endif
                                        </div>
                                ...
                                @empty
                                <tr>
                                    <td colspan="4" class="text-center py-4">Entitas ini belum mengunggah dokumen
                                        apapun.</td>
                                </tr>
                                @endforelse
                            </tbody>
