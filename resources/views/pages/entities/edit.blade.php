                    ...
                    <form action="{{ route('entities.update', $entity->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="type" id="type"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                required>
                                <option value="OPD" {{ old('type', $entity->type) == 'OPD' ? 'selected' : '' }}>OPD
                                </option>
                                <option value="Kecamatan"
                                    {{ old('type', $entity->type) == 'Kecamatan' ? 'selected' : '' }}>Kecamatan</option>
                                <option value="Desa" {{ old('type', $entity->type) == 'Desa' ? 'selected' : '' }}>Desa
                                </option>
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="parent_id" id="parent_id"
                                ...
                                <option value="{{ $parent->id }}"
                                    {{ old('parent_id', $entity->parent_id) == $parent->id ? 'selected' : '' }}>
