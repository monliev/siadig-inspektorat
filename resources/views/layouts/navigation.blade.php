<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="shrink-0 flex items-center">
                    <a href="{{ Illuminate\Support\Facades\Gate::allows('isClient') ? route('client.dashboard') : route('dashboard') }}">
                        ...
                        <div class="hidden sm:flex sm:items-center sm:ms-6">
                            <x-dropdown align="right" width="48">
                                ...
                                <x-slot name="content">
                                    ...
                                    <x-dropdown-link :href="route('documents.client_submissions')" :active="request()->routeIs('documents.client_suissions')">
    ...
    <div class="pt-2 pb-3 space-y-1">
        ...
        @else
            ...
            <x-responsive-nav-link :href="route('documents.index')" :active="request()->routeIs('documents.index') && !request()->routeIs('documents.reviewList')">
                {{ __('Arsip Internal') }}
            </x-responsive-nav-link>

            @can('isAdmin')
                <x-responsive-nav-link :href="route('documents.reviewList')" :active="request()->routeIs('documents.reviewList')">
                    {{ __('Review Dokumen Internal') }}
                </x-responsive-nav-link>

                {{-- Dropdown Permintaan Dokumen OPD --}}
                <details class="px-4 py-2">
                    <summary class="cursor-pointer text-gray-700 font-medium">Permintaan Dokumen OPD</summary>
                    <div class="ml-4 mt-2 space-y-1">
                        <x-responsive-nav-link :href="route('document-requests.index')">
                            {{ __('Permintaan Dokumen') }}
                        </x-responsive-nav-link>
                        <x-responsive-nav-link :href="route('documents.client_submissions')" :active="request()->routeIs('documents.client_submissions')">
                            {{ __('Daftar Dokumen OPD') }}
                        </x-responsive-nav-link>
                    </div>
                </details>
                ...
                <details class="px-4 py-2">
                    <summary class="cursor-pointer text-gray-700 font-medium">Master Data</summary>
                    <div class="ml-4 mt-2 space-y-1">
                        <x-responsive-nav-link :href="route('document-categories.index')">
                            {{ __('Kategori Dokumen') }}
                        </x-responsive-nav-link>
                        ...
                        <x-responsive-nav-link :href="route('entities.index')">
                            {{ __('Manajemen Entitas') }}
                        </x-responsive-nav-link>
                    </div>
                </details>
